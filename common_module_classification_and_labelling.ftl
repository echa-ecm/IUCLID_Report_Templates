...
<@com.initiateRelevanceSPC relevance/>
...
<#assign recordList = iuclid.getSectionDocumentsForParentKey(_subject.documentKey, "FLEXIBLE_RECORD", "Ghs") />
    ...
    <#else>
        ...
        <#list recordList as record>
            <@printClassificationSection record record_index/>
			<!-- Labelling section -->
            <@printLabellingSection record/>
...
<#macro printClassificationSection record record_index>
    <#if csrRelevant?? || pppRelevant?? || !ghsRelevant??>
		<para>
            <@com.emptyLine/>

			<emphasis role="bold">
				<emphasis role="underline"><#if pppRelevant?? && (recordList?size>1)>#${record_index+1} - </#if>
					Substance: <@classificationSubstanceName record _subject/></emphasis>
			</emphasis>
		</para>
        ...
        <#if pppRelevant?? && record.GeneralInformation.TypeClassification?has_content>
			<para>
				<emphasis role="bold">Type of classification:
				</emphasis> <@com.picklist record.GeneralInformation.TypeClassification/>
			</para>
        </#if>
        ...
        <#if record.GeneralInformation.RelatedCompositions.Composition?has_content>
			<para>
                <#if pppRelevant??>
					<emphasis role="bold">Related composition:</emphasis>
					<emphasis
							role="underline"><@com.documentReferenceMultiple record.GeneralInformation.RelatedCompositions.Composition/></emphasis>
                <#else>
					<emphasis role="underline">Related
						composition: <@com.documentReferenceMultiple record.GeneralInformation.RelatedCompositions.Composition/></emphasis>
                </#if>
			</para>
        </#if>
    ...
    <#if pppRelevant??>
        ...
        <#else>
			<para>
				<emphasis role="bold">Classification</emphasis>
			</para>
    ...
    <@com.emptyLine/>
	<@printClassificationForPhysicochemicalPropertiesTable record />
...
<#-- AICIS only docbook format and data -->
    ...
    <@com.emptyLine/>
	<@printClassificationForHealthHazardsTable record/>
			...
			<informaltable border="1">
				...
				<tr>
					<th><?dbfo bgcolor="#FBDDA6" ?>
						<emphasis role="bold">Concentration (%)</emphasis>
					</th>
					<th><?dbfo bgcolor="#FBDDA6" ?>
						<emphasis role="bold">Classification</emphasis>
					</th>
...
<#-- AICIS only docbook format and data -->
    ...
    <@com.emptyLine/>
	<@printClassificationForEnvironmentalHazards record/>
...
<#macro printLabellingSection record>
    <#if !nzEPArelevant??>
        <@com.emptyLine/>
...
<#macro HazardPictogramList HazardPictogramRepeatableBlock shouldShowPictureWithIndent=true>
<#compress>
	<#if HazardPictogramRepeatableBlock?has_content>
		<#list HazardPictogramRepeatableBlock as blockItem>
			<#if shouldShowPictureWithIndent>
				<para role="indent">
			<#else>
				<para>
			</#if>
...
<#macro printClassificationForPhysicochemicalPropertiesTable record>
	<table border="1">
        <#if nzEPArelevant??>
        ...
        <#if nzEPArelevant??>
			<tr>
				<th><?dbfo bgcolor="#FBDDA6" ?>
					<emphasis role="bold">Hazard class</emphasis>
				</th>
				<th><?dbfo bgcolor="#FBDDA6" ?>
					<emphasis role="bold">Hazard category</emphasis>
				</th>
				<th><?dbfo bgcolor="#FBDDA6" ?>
					<emphasis role="bold">Hazard statement</emphasis>
				</th>
			</tr>
        <#else>
			<tr>
				...
				<th><?dbfo bgcolor="#FBDDA6" ?>
					<emphasis role="bold">Reason for no classification</emphasis>
				</th>
			</tr>
        </#if>
...
<#macro printClassificationForHealthHazardsTable record>
	<table border="1">
        <#if nzEPArelevant??>
...
<#macro printClassificationForEnvironmentalHazards record>
	<table border="1">
        <#if nzEPArelevant??>
        ...
        <#if nzEPArelevant??>
            <#if record.Classification.EnvironmentalHazards.AquaticEnvironment.MFactor.MFactorAcute?has_content>
				<tr>
					<td colspan="3">
						M-Factor
						acute: <@com.number record.Classification.EnvironmentalHazards.AquaticEnvironment.MFactor.MFactorAcute/>
					</td>
				</tr>
            </#if>
        ...
        <#if nzEPArelevant??>
            <#if record.Classification.EnvironmentalHazards.AquaticEnvironment.MFactor.MFactorChronic?has_content>
				<tr>
					<td colspan="3">
						M-Factor
						chronic: <@com.number record.Classification.EnvironmentalHazards.AquaticEnvironment.MFactor.MFactorChronic/>
					</td>
				</tr>
            </#if>
